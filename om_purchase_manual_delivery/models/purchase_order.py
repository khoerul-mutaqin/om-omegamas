...
class PurchaseOrder(models.Model):
    ...
    is_fully_received_or_billed = fields.Char(compute='_compute_is_fully_received_or_billed', string='Fully Receieved / Billed')
    is_closed = fields.Boolean('Closed PO', default=False, copy=False)


    @api.depends('order_line')
    def _compute_is_fully_received_or_billed(self):
        for rec in self:
            rec.is_fully_received_or_billed = all([line.product_qty == line.qty_received and line.product_qty == line.qty_invoiced for line in rec.order_line.filtered(lambda x: not x.is_deposit) ])
...
class StockPicking(models.Model):
    ...
    date_done = fields.Datetime(string='Date Done', compute='_compute_date_done', store=True)
