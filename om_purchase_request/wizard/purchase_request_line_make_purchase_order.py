...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env["purchase.request.line"].browse(request_line_ids):
            ...
            if line.purchase_state == "rejected":
                raise UserError(
                    _("Purchase Request %s one of the lines is Rejected") % line.request_id.name
                )
    ...
    @api.model
    def _get_order_line_search_domain(self, order, item, uom_pr_line):
        ...
        order_line_data = [
            ...
            ("product_uom", "=", uom_pr_line.id)
            # ("analytic_distribution", "=?", item.line_id.analytic_distribution),
        ]
        # if item.line_id.analytic_distribution:
        #     order_line_data.append(("analytic_distribution", "=", item.line_id.analytic_distribution))
