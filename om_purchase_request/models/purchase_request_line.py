...
class PurchaseRequestLine(models.Model):
    ...
    def action_show_details(self):
        self.ensure_one()
        view = self.env.ref("om_purchase_request.view_purchase_request_line_details")
        return {
    ...
    def button_rejected(self):
        ...
        if self.request_id.state != "approved":
            raise UserError(
                _("Purchase Request %s is not approved") % self.request_id.name
            )
        self.write({'request_state': 'rejected', 'cancelled': True, 'purchase_state': 'cancel'})
        return True
