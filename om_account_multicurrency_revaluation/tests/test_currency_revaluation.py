...
class TestCurrencyRevaluation(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        payable_acc = cls.env.ref("om_account_multicurrency_revaluation.demo_acc_payable")
    ...
    def test_revaluation_payment(self):
        ...
        acc_suspense = self.env.ref(
            "om_account_multicurrency_revaluation.demo_acc_suspense"
        )
    ...
    def update_company(self, with_analytic=False, **options):
        if with_analytic:
            for field, xml_id in [
                ...
                xml_id = "om_account_multicurrency_revaluation." + xml_id
                options[field] = self.env.ref(xml_id)
        self.company.write(options)

    def create_invoice(self, date, currency, quantity, price):
        revenue_acc = self.env.ref("om_account_multicurrency_revaluation.demo_acc_revenue")
        payment_term = self.env.ref("om_account.account_payment_term_end_following_month")
        sales_journal = self.env.ref("om_account_multicurrency_revaluation.sales_journal")
        invoice_line_data = {
